...
{
    fd_ = kqueue();
    ErrorIf(fd_ == -1, "kequeue create error");
...
void Poller::UpdateChannel(Channel* ch)
{
    ...
    int ret = kevent(fd_, ev, n, NULL, 0, NULL);
    ErrorIf(ret == -1, "kqueue add event error");
}
...
void Poller::DeleteChannel(Channel* ch)
{
    ...
    int ret = kevent(fd_, ev, n, NULL, 0, NULL);
    ErrorIf(ret == -1, "kqueue delete event error");
}
...
std::vector<Channel*> Poller::Poll(int timeout)
{
    ...
    if (timeout != -1)
    {
        ...
        ts.tv_sec = (timeout % 1000) * 1000 * 1000;
    }
